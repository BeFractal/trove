...
docker run --rm --name eosio_trovechain_container -d \
...
if [ "$1" != "--nolog" ]
then
    echo "=== follow eosio_trovechain_container logs ==="
    docker logs eosio_trovechain_container --follow
fi
