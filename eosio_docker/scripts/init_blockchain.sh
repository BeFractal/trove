...
echo "=== setup wallet: trovechainwal ==="
# key for eosio account and export the generated password to a file for unlocking wallet later
cleos wallet create -n trovechainwal --to-console | tail -1 | sed -e 's/^"//' -e 's/"$//' > trovechain_wallet_password.txt
# Owner key for notechainwal wallet
cleos wallet import -n trovechainwal --private-key 5JpWT4ehouB2FF9aCfdfnZ5AwbQbTtHBAwebRXt94FmjyhXwL4K
# Active key for notechainwal wallet
cleos wallet import -n trovechainwal --private-key 5JD9AGTuTeD5BXZwGQ5AtwBqHK21aHmYnTetHgk1B3pjj7krT8N

# * Replace "trovechainwal" by your own wallet name when you start your own project

# create account for notechainacc with above wallet's public keys
cleos create account eosio troveacc EOS6PUh9rs7eddJNzqgqDx1QrspSHLRxLMcRdwHZZRL4tpbtvia5B EOS8BCgapgYA2L4LJfCzekzeSr3rzgSTUXRXwNi8bNRoz31D14en9
...
deploy_contract.sh trovechain troveacc trovechainwal $(cat notechain_wallet_password.txt)
